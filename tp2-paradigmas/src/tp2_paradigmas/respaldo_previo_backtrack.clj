(ns tp2-paradigmas.respaldo-previo-backtrack)
...
(defn tranformate-1 [reglas-state]
  (let [reglas (reglas-state 0)
        ...
        new-state (->> state
                       ...
                       (apply str,,,))]
    [reglas new-state]))
;str axioma y mapa reglas
;recive la cantidad de veces y un vector reglas-axioma
...
(defn calcular-x [angulo h]
  (* (Math/cos (anguloReal angulo)) h))

(defn calcular-y [angulo h]
  (* (Math/sin (anguloReal angulo)) h))

(defn suma-vec [v1 v2]
  (vec (map numero-dos-decimales (map + v1 v2))))

(defn mov-x-y [angulo h]
  (vec (list (calcular-x angulo h) (calcular-y angulo h))))
...
(defn create-estado
  ([lineas pila t-actual mx mn] {:lineas lineas :pila pila :t-actual t-actual :mx mx :mn mn})
  ([] (create-estado '() '() (crear-tortuga [[0 0] 0]) [0 0] [0 0])))

(defn get-line
  ([modo x y ] (str modo " " x " " y))
  ([modo tortuga] (let [x ((tortuga :pos )0) y ((tortuga :pos )1)] (get-line modo x y))))

(defn update-manejo-pila [new-pila new-line estado]
  (-> estado
      (assoc ,,, :lineas (concat (estado :lineas) '(lisnew-line)))
      (assoc ,,, :pila new-pila)
      (assoc ,,, :t-actual (first new-pila))))

(defn update-avanzar [new-tortuga modo estado]
  (let [new-line (get-line modo new-tortuga)]
    (do (println (str "nueva linea " new-line "con modo " modo))(-> estado
                                                                    ...
                                                                    (assoc ,,, :mx (#(mapv max %1 %2) (new-tortuga :pos) (estado :mx)))
                                                                    (assoc ,,, :mn (#(mapv min %1 %2) (new-tortuga :pos) (estado :mn)))))
    ))
...
;esto sirve parra avanzar y para girar
...
(defn updater-estado [f type-action & modo]
  (fn [n estado] (let [new-tortuga (f (estado :tortuga) n)]
                   (case type-action
                     :avanzar (update-avanzar new-tortuga modo estado)
                     :girar (update-girar new-tortuga estado)))))

;debe ser una función que reciba n y estado y devuelva estado
(defn create-manejo-pila [apilo?]
  (fn [n estado]
    (let [new-pila (case apilo?
                     true (apilar-n (estado :pila) (estado :tortuga) n)
...
(defn create-girar [ang, sentido]
  "crea un función de giro (que recibe cuantas repeticiones de girar hay y el estado inicial de la tortuga, devuelve una tortuga nueva) recibe un angulo y un sentido (-1 si es horario, 1 si es horario)"
  (fn [tortuga n]
    (->> (* n sentido ang)
         (+ (tortuga :angulo),,,)
         (vec)
...
;recibe tortuga y data (cantidad de pasos a dar) y devuelve tortuga actualizada
(defn avanzar [tortuga n]
  "recibe cuantas repeticiones de avanzar hay y el estado inicial de la tortuga, devuelve una tortuga nueva)"
  (->> (* 10 n)
       (mov-x-y (tortuga :angulo))
       (suma-vec (tortuga :pos))
       (conj [tortuga :angulo],,,)
       (reverse)
       (vec)))
...
;devuelve una función que espera por estado y devuelve un nuevo estado
(defn convert-to-function [seq-op-codes sistem]
  (as-> seq-op-codes codes
        (first codes)                                       ;primer
        ((create-alfabeto sistem) codes)                       ;aplica el alfabeto a una letra y obtiene una función updater (función que espera por datos del código en particular y estado )
        (partial codes (count seq-op-codes))))              ;funcion que solo espera por estado
